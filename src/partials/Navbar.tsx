...
const Navbar = () => {
  ...
  const scrollTo = (event: MouseEvent<HTMLAnchorElement>) => {
    ...
    const section = document.getElementById(id) ?? document.body;
  ...
  return (
    <header
      ref={header}
      className="fixed z-[999] flex h-0
      w-11/12
      flex-row-reverse items-center justify-between gap-y-3 text-lg text-white opacity-0 mix-blend-difference md:flex-row"
    >
      <NavMenu>
      ...
      <div className="h-0 md:w-1/3">
        <a
          id="logo"
          href="/#top"
          title="Waves Web Studio Homepage"
          className="mx-auto flex h-10 w-[105px] translate-y-2 md:h-6 md:w-fit md:translate-y-9"
          onClick={scrollTo}
        >
          <svg
            ...
            version="1.0"
            viewBox="0 0 119 27"
            className="pointer-events-none h-auto self-center fill-white"
          >
      ...
      <div className="hidden h-0 w-1/3 justify-end md:flex">
        <UnderlinedLink href={schedulerLink} className="size-fit translate-y-9">
          Schedule a Call
        </UnderlinedLink>
      </div>
    </header>
